...
This choice greatly reduced the computational cost of sampling low-energy states as linear optimization was not necessary for every sampled state.
...
This is seen in a huge energy gap of $\epsilon_{2p_\pi} - \epsilon_{3d_\delta} = $ 5.87 eV, which is 2 $-$ 3 eV in experiment and our sampled data.
...
I have shown that my model has comparable accuracy to modern computational techniques.
This poses the question, however: why use DMD if simpler methods are just as accurate?
The next steps in the project will address this question by demonstrating the unique interpretability of a model.
I plan on accomplishing this by refitting my model after dropping specific interactions, like $n_{4s,\uparrow} n_{4s,\downarrow}$, and quantitatively evaluate the effect on the model spectra, eigenstates, and renormalization of non-interacting parameters.
In doing so, I will be able to describe the precise effects of certain interactions on the low-energy spectra of CuO, a level of detailed interpretation unavailable to methods like CCSD(T). 
...
Phononic contributions will be incorporated by fitting a sequence of models to altered geometries, leading to displacement dependent parameters similar to a spin-phonon model for the cuprates \cite{Wagner2014}.
...
The first three months will be dedicate to reproducing the on-site Hubbard model from that work.
