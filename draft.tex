...
The results of my thesis will contribute significantly to the understanding of low-energy effective interactions in single and bilayer graphene, and will provide a baseline for continuing study of the twisted bi-layer graphene (TBLG) system.
To demonstrate my preparedness I present progress on a many-body model for the CuO molecule and my work in QMC methods development.
\end{abstract}
...
Instead of jumping head first into TBLG, however, I propose a two year plan for modeling the AA and AB stacked BLG systems using DMD with \textit{ab initio} fixed-node diffusion Monte Carlo (FN-DMC) calculations.
I will first develop a model for single-layer graphene (SLG) with long-range Coulomb and electron-phonon interactions, and then extend this to the BLG systems by introducing inter-layer couplings.
The SLG model will demonstrate the importance of long range interactions and geometric relaxation in describing the low-energy excitations of graphene.
...
I have shown that the my model has comparable accuracy to modern computational techniques.
This poses the question, however: why use DMD if one can reproduce eigenstates with simpler methods? 
The next steps in the project will address this question by demonstrating the unique interpretability of a model.
I plan on accomplishing this by refitting my model after dropping specific interactions, like $n_{4s,\uparrow} n_{4s,\downarrow}$, and quantitatively describe the effect on the model spectra and eigenstates, as well as renormalization of non-interacting parameters.
In doing so, I will be able to categorize the precise effects of certain interactions on the low-energy spectra of CuO, a level of detailed interpretation unavailable to methods like CCSD(T). 
...
Unfortunately, a naive Monte Carlo estimate of the gradient has an infinite fourth moment due to divergences near the nodal surface of $|\Psi \rangle$, so MC error estimates for the gradient are very unstable.
...
I also made significant contributions to a new QMC package called PyQMC. 
This package is oriented towards methods development, and will be the platform for most of my future DMD calculations.
I implemented multi-Slater wave functions, ran consistency checks against QWalk, and worked on execution speedups for costly methods like VMC and wave function optimization.
...
Phononic contributions will be incorporated by fitting a sequence of models to altered geometries, leading to displacement dependent parameters similar to a previous spin-phonon calculation \cite{Wagner2014}.

An important result from this study would be an accurate evaluation of $V(r)$ directly from \textit{ab initio} simulations.
This result would be able to validate commonly used models for the spatial dependence, like the Ohno form \cite{Ohno1964}.
Further, it will help develop understanding of interaction effects in graphene which depend on the details of $V(r)$, like edge magentism \cite{PhysRevB.95.195420} and non-local electrodynamics \cite{PhysRevB.90.045137}.
Additionally, by introducing displacement dependent parameters, one can determine whether phonons in graphene serve to simply renormalize interactions, or drastically alter them.
...
Lastly, when combined with the SLG model, it will be possible to determine how the intra-layer model is affected by the presence of a neighboring graphene sheet.
The simple BLG models will also enable follow-up studies on the twist dependence of model parameters for TBLG.
...
In total, my work will contribute significantly to the understanding of low-energy effective interactions in single and bilayer graphene.
