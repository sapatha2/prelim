...
Instead, model validation rely on comparisons to experiment.
...
%Rework this paragraph, could be much better with the new idea i have!
Instead of jumping head on into TBLG, however, I propose a two year plan for modeling the AA and AB stacked BLG systems using DMD with \textit{ab initio} fixed-node diffusion Monte Carlo (FN-DMC) calculations.
...
The SLG model will serve as an important tool in understanding the interplay of electronic and geometric degrees of freedom in graphene, as seen in straining experiments \cite{Naumis_2017}.
...
My work will also provide a baseline for follow-ups on the effects of commensurate twists on inter- and intra-layer coupling in TBLG.
